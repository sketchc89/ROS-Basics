...
g_vel_scales = [0.1, 0.1] # default to slow
g_vel_ramps = [1,1] # m/s^2

def ramped_vel(v_prev, v_target, t_prev, t_now, ramp_rate):
    '''Compute max velocity step. Steps until error is less than step size'''
...
def ramped_twist(prev, target, t_prev, t_now, ramps):
    '''Accelerate from prev to target at ramps rate'''
    tw = Twist()
...
def send_twist():
    '''Publish result of ramped twist'''
...
def keys_cb(msg):
    '''Set target twist based on key pressed'''
...
def fetch_param(name, default):
    '''Check whether parameter exists and if not return default'''
    if rospy.has_param(name):
        return rospy.get_param(name)
    else:
        print('Parameter {0} not defined. Defaulting to {1}'.format(name, default))
        return default
